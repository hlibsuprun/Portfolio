...
describe('clickHandler', () => {
  ...
  it('when there is no first number', () => {
    ...
    expect(new Plus(expression, 0).clickHandler()).toEqual(expression)
  })

  it('when there is one number', () => {
    expression = { strFirstNumber: '1', sign: '', strSecondNumber: '' }

    expect(new Plus(expression, 1).clickHandler()).toEqual({ ...expression, sign: '+' })
  })
  ...
  it('when there is full expression', () => {
    ...
    expect(new Plus(expression, 1).clickHandler()).toEqual({ ...expression, sign: '+', strSecondNumber: '' })
  })
})
